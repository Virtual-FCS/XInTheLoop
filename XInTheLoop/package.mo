...
<p>In the <a href=\"modelica://XInTheLoop.Examples.Site1\">Site1 example</a>, the hardware is a Fuel Cell System (FCS) that consists of a Fuel Cell, a DC-DC-converter, a battery, and a variable load. The UDP protocol between the model and the low-level hardware controller is adapted to this, but any user can make customized versions to support other external systems by reusing a set of generic blocks.</p>
<p>The <a href=\"modelica://XInTheLoop.Examples.Site2\">Site2 example</a> contains test models that use a UDP protocol to exchange values with an external system running a StasHH compatible fuel cell module (FCM), where a fuel cell control unit (FCCU) handle the StasHH Digital Interface via a CAN bus, and python scripts acting as a relay between the UDP protocol and the CAN protocol.</p>
<h4>Known Issue</h4>
...
<tr><td>1.0</td><td>2024-11-14</td>
<td>Add StasHH use case as Site2 example.</td></tr>
